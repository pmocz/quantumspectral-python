...
def main():
    ...
    rho += (
        2
        * amp
        * np.exp(-((xx - 0.5) ** 2 + (yy - 0.5) ** 2) / 2 / sigma**2)
        / (sigma**3 * np.sqrt(2 * np.pi) ** 2)
    )
    rho += (
        1.5
        * amp
        * np.exp(-((xx - 0.2) ** 2 + (yy - 0.7) ** 2) / 2 / sigma**2)
        / (sigma**3 * np.sqrt(2 * np.pi) ** 2)
    )
    rho += (
        amp
        * np.exp(-((xx - 0.4) ** 2 + (yy - 0.6) ** 2) / 2 / sigma**2)
        / (sigma**3 * np.sqrt(2 * np.pi) ** 2)
    )
    rho += (
        amp
        * np.exp(-((xx - 0.6) ** 2 + (yy - 0.8) ** 2) / 2 / sigma**2)
        / (sigma**3 * np.sqrt(2 * np.pi) ** 2)
    )
    rho += (
        amp
        * np.exp(-((xx - 0.8) ** 2 + (yy - 0.2) ** 2) / 2 / sigma**2)
        / (sigma**3 * np.sqrt(2 * np.pi) ** 2)
    )
    rho += (
        amp
        * np.exp(-((xx - 0.6) ** 2 + (yy - 0.7) ** 2) / 2 / sigma**2)
        / (sigma**3 * np.sqrt(2 * np.pi) ** 2)
    )
    rho += (
        amp
        * np.exp(-((xx - 0.7) ** 2 + (yy - 0.4) ** 2) / 2 / sigma**2)
        / (sigma**3 * np.sqrt(2 * np.pi) ** 2)
    )
    rho += (
        amp
    ...
    for i in range(Nt):
        # (1/2) kick
        psi = np.exp(-1.0j * dt / 2.0 * V) * psi

        # drift
        psihat = np.fft.fftn(psi)
        psihat = np.exp(dt * (-1.0j * kSq / 2.0)) * psihat
        psi = np.fft.ifftn(psihat)

        # update potential
        Vhat = -np.fft.fftn(4.0 * np.pi * G * (np.abs(psi) ** 2 - 1.0)) / (
            kSq + (kSq == 0)
        )
        V = np.real(np.fft.ifftn(Vhat))
